...
## 2024-07-06 - SSRF/LFI via Unvalidated urllib.request.urlopen
**Vulnerability:** Multiple instances of `urllib.request.urlopen` across scripts (`qoe_probe.py`, `setup_portable.py`, `update_qoe_probe.py`) lacked explicit URL scheme validation (Bandit B310). This could potentially allow Server-Side Request Forgery (SSRF) or Local File Inclusion (LFI) via `file://` or custom schemes if user-controlled input reached these sinks.
**Learning:** Python's standard `urllib.request.urlopen` supports multiple schemes by default, not just HTTP/HTTPS. When fetching data based on external configurations, explicitly validating that the scheme is permitted is crucial to prevent unexpected file access or internal network requests.
**Prevention:** Always explicitly validate that URLs start with `http://` or `https://` (case-insensitive) before passing them to `urllib.request.urlopen`. To silence false-positive static analysis warnings (e.g., Bandit B310) after implementing this validation, append `# nosec B310` to the specific line.
