...
def register_windows(
    ...
    $action = New-ScheduledTaskAction -Execute $env:CX_RADAR_SYS_EXE -Argument "`"$env:CX_RADAR_SCRIPT_PATH`" --config-path `"$env:CX_RADAR_CONFIG_PATH`""
    $trigger = New-ScheduledTaskTrigger -Once -At ((Get-Date).Date) -RepetitionInterval (New-TimeSpan -Minutes $env:CX_RADAR_INTERVAL)
    ...
    $task = New-ScheduledTask -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description $env:CX_RADAR_DESCRIPTION
    ...
    env_vars["CX_RADAR_SYS_EXE"] = sys.executable
    env_vars["CX_RADAR_SCRIPT_PATH"] = str(script_path)
    env_vars["CX_RADAR_CONFIG_PATH"] = str(config_path)
    env_vars["CX_RADAR_INTERVAL"] = str(interval_minutes)
    env_vars["CX_RADAR_LOGON_TYPE"] = logon_type
    env_vars["CX_RADAR_TASK_NAME"] = task_name
    env_vars["CX_RADAR_DESCRIPTION"] = description
