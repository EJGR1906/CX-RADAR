...
## 2026-07-14 - SSRF and LFI via unvalidated urllib.request.urlopen
**Vulnerability:** A medium severity vulnerability (Bandit B310) was present across `scripts/qoe_probe.py`, `scripts/setup_portable.py`, and `scripts/update_qoe_probe.py` where `urllib.request.urlopen` was called with unvalidated URLs. This could potentially allow Server-Side Request Forgery (SSRF) or Local File Inclusion (LFI) via `file://` or custom schemes if user input (e.g., config data) flows into the URL.
**Learning:** Python's `urllib.request.urlopen` supports arbitrary schemes like `file://` out of the box, which introduces significant risk when fetching data dynamically. Bandit flags these as B310 because it cannot verify runtime URL contents statically.
**Prevention:** Always manually validate that a dynamically provided URL string starts with `http://` or `https://` (case-insensitively) before calling `urlopen`. When doing so, you must explicitly append `# nosec B310` to the `urlopen` invocation to silence Bandit's static analysis warning, as it does not recognize the runtime validation check.
