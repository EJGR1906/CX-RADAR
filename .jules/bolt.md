...
## 2026-07-15 - [Refused Micro-optimization: regex in I/O bound path]
**Learning:** Extracting `re.search` calls into module-level `re.compile` declarations inside a function that parses the output of a slow, I/O bound external network tool (like `networkquality`, which takes seconds to run) yields virtually zero real-world performance benefit. Because Python caches the most recent 512 regex patterns automatically, the cache lookup overhead is negligible (nanoseconds) compared to the tool's execution time. Applying this pattern outside of tight, high-frequency CPU loops violates the "avoid meaningless micro-optimizations" boundary.
**Action:** Do not pre-compile regular expressions if they are used to parse slow, I/O bound external command outputs. Reserve this optimization pattern exclusively for tight, high-frequency iterative processing loops (e.g., parsing thousands of lines per second).
