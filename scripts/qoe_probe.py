...
# NetworkQuality Regexes
_NQ_DL_REGEX_1 = re.compile(r'Download(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
_NQ_DL_REGEX_2 = re.compile(r'Down(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
_NQ_UL_REGEX_1 = re.compile(r'Upload(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
_NQ_UL_REGEX_2 = re.compile(r'Up(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
_NQ_LAT_REGEX_1 = re.compile(r'(?:Idle\s+)?Latency\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*ms', re.I)
_NQ_LAT_REGEX_2 = re.compile(r'Round\s*trip\s*latency\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*ms', re.I)
_NQ_RPM_REGEX_1 = re.compile(r'Responsiveness\s*:\s*.*?\(([0-9]+(?:\.[0-9]+)?)\s*RPM\)', re.I)
_NQ_RPM_REGEX_2 = re.compile(r'RPM(?:\s+Responsiveness)?\s*:\s*([0-9]+(?:\.[0-9]+)?)', re.I)
...
def run_networkquality_measurement(
    ...
    if timed_out:
        ...
        try:
            ...
            combined = stdout + "\n" + stderr
            # Download
            m_dl = _NQ_DL_REGEX_1.search(combined)
            if not m_dl:
                m_dl = _NQ_DL_REGEX_2.search(combined)
            if m_dl:
                download_speed = float(m_dl.group(1))

            # Upload
            m_ul = _NQ_UL_REGEX_1.search(combined)
            if not m_ul:
                m_ul = _NQ_UL_REGEX_2.search(combined)
            if m_ul:
                upload_speed = float(m_ul.group(1))

            # Latency
            m_lat = _NQ_LAT_REGEX_1.search(combined)
            if not m_lat:
                m_lat = _NQ_LAT_REGEX_2.search(combined)
            if m_lat:
                latency = float(m_lat.group(1))

            # RPM
            m_rpm = _NQ_RPM_REGEX_1.search(combined)
            if not m_rpm:
                m_rpm = _NQ_RPM_REGEX_2.search(combined)
            if m_rpm:
