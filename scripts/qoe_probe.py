...
# ⚡ Bolt: Pre-compiled regex patterns for networkquality parser to avoid re-compilation overhead
NQ_DOWNLOAD_REGEX = re.compile(r'Download(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_DOWNLINK_REGEX = re.compile(r'Down(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_UPLOAD_REGEX = re.compile(r'Upload(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_UPLINK_REGEX = re.compile(r'Up(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_LATENCY_REGEX = re.compile(r'(?:Idle\s+)?Latency\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*ms', re.I)
NQ_RTT_LATENCY_REGEX = re.compile(r'Round\s*trip\s*latency\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*ms', re.I)
NQ_RPM_REGEX = re.compile(r'Responsiveness\s*:\s*.*?\(([0-9]+(?:\.[0-9]+)?)\s*RPM\)', re.I)
NQ_RPM_FALLBACK_REGEX = re.compile(r'RPM(?:\s+Responsiveness)?\s*:\s*([0-9]+(?:\.[0-9]+)?)', re.I)
...
def run_networkquality_measurement(
    ...
    if timed_out:
        ...
        try:
            ...
            combined = stdout + "\n" + stderr
            # Download
            m_dl = NQ_DOWNLOAD_REGEX.search(combined)
            if not m_dl:
                m_dl = NQ_DOWNLINK_REGEX.search(combined)
            if m_dl:
                download_speed = float(m_dl.group(1))

            # Upload
            m_ul = NQ_UPLOAD_REGEX.search(combined)
            if not m_ul:
                m_ul = NQ_UPLINK_REGEX.search(combined)
            if m_ul:
                upload_speed = float(m_ul.group(1))

            # Latency
            m_lat = NQ_LATENCY_REGEX.search(combined)
            if not m_lat:
