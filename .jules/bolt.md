...
## 2026-07-09 - Pre-compile regular expressions
**Learning:** Inline regex compilation (e.g., `re.search(r'pattern', string)`) inside loops or frequently called parsing functions incurs repeated compilation overhead and cache lookups.
**Action:** Always move `re.compile` declarations to the global module level so they are evaluated exactly once when the script loads.
