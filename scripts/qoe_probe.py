...
# Networkquality Speed Test outputs
NQ_DL_CAPACITY_REGEX = re.compile(r'Download(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_DL_BW_REGEX = re.compile(r'Down(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_UL_CAPACITY_REGEX = re.compile(r'Upload(?:\s+capacity)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
NQ_UL_BW_REGEX = re.compile(r'Up(?:link)?(?:\s+bandwidth)?\s*:\s*([0-9]+(?:\.[0-9]+)?)\s*([A-Za-z/]+)', re.I)
...
def run_networkquality_measurement(
    ...
    if timed_out:
        ...
        try:
            ...
            if not m_dl:
                m_dl = NQ_DL_BW_REGEX.search(combined)
            if m_dl:
            ...
            if not m_ul:
                m_ul = NQ_UL_BW_REGEX.search(combined)
            if m_ul:
