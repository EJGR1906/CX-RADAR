...
> [!NOTE]
> **Aviso de Migración**: Este runbook ha sido actualizado para la nueva implementación en Python 3 (Multiplataforma). Para más detalles sobre las decisiones de diseño y equivalencia de archivos, consulte la [Guía de Migración](file:///c:/Users/Eduar/Documents/CX-RADAR/docs/MIGRATION.md).

## Resumen

Este runbook es la ruta de QA ejecutable para la sonda HTTP sintética multiplataforma (Python 3).
...
```bash
python scripts/run_qoe_certification.py --run-resilience-checks --include-tls-scenario --output-path logs/qoe-certification-tls.json
```
...
2. Registra los valores base de working set de `python.exe` (o `python`), recuento de handles y recuento de procesos antes del soak.
...
1. No hay procesos persistentes `curl` (o `curl.exe`) huérfanos
2. No hay crecimiento monótono en el recuento de procesos de Python relacionados con la sonda
...
1. La salida de smoke de `run_qoe_certification.py` pasa.
2. `run_qoe_certification.py --run-resilience-checks` pasa.
3. El almacenamiento de token por variable de entorno o archivo `.env` está configurado.
4. Al menos una escritura real a InfluxDB tiene éxito en Python 3.
